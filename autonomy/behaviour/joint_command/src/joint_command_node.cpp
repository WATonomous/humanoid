...
JointCommandNode::JointCommandNode() : Node("joint_command_node") {
  ...
  this->declare_parameter("control_type",
                          common_msgs::msg::MotorCmd::POSITION_LOOP);
  ...
  const std::string motor_cmd_topic =
      this->get_parameter("motor_cmd_topic").as_string();
  control_type_ =
      static_cast<int8_t>(this->get_parameter("control_type").as_int());

  const YAML::Node hardware_config = YAML::LoadFile(
  ...
  RCLCPP_INFO(this->get_logger(),
              "Joint command node ready: arm=%s, joints=%zu, rate=%.1f Hz",
              arm_side.c_str(), core_.jointCount(), control_rate_hz_);
}

void JointCommandNode::armPoseCallback(
    const common_msgs::msg::ArmPose::SharedPtr msg) {
  try {
...
void JointCommandNode::publishMotorCommands(
    const std::vector<common_msgs::msg::MotorCmd> &cmds) {
