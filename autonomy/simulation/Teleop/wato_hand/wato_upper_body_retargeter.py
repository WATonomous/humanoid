...
# This import exception is suppressed because wato_dex_retargeting_utils depends
# on pinocchio which is not available on Windows.
with contextlib.suppress(Exception):
    from .wato_dex_retargeting_utils import WatoHandDexRetargeting


class WatoUpperBodyRetargeter(RetargeterBase):
    """Retargets OpenXR data to Wato arm_assembly upper body commands.

    This retargeter maps hand tracking data from OpenXR to wrist and hand joint commands for the Wato arm_assembly hand.
    ...
    def __init__(
        self,
        cfg: WatoUpperBodyRetargeterCfg,
    ):
        """Initialize the Wato upper body retargeter.
        ...
        if cfg.hand_joint_names is not None:
            self._hands_controller = WatoHandDexRetargeting(cfg.hand_joint_names)
        else:
...
@dataclass
class WatoUpperBodyRetargeterCfg(RetargeterCfg):
    enable_visualization: bool = False
    num_open_xr_hand_joints: int = 100
    hand_joint_names: list[str] | None = None  # List of robot hand joint names
    retargeter_type: type[RetargeterBase] = WatoUpperBodyRetargeter
