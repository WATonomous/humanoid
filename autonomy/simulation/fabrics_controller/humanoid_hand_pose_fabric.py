...
class HumanoidHandPoseFabric(BaseFabric):
    ...
    def add_hand_fabric(self):
        ...
        pca_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pca_matrix.npy")
        pca_numpy = np.load(pca_path)

        # Transpose to (7, 15) — each row is one principal component
        pca_matrix = torch.tensor(pca_numpy.T, device=self.device, dtype=torch.float32)
