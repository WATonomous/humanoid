...
# Compute actual score ranges from training data so run_example.py can use
# data-driven per-component bounds (instead of a symmetric fixed range).
scores = pca.transform(Q_clipped)[:, :k]   # (N, k)
score_mins = scores.min(axis=0)
score_maxs = scores.max(axis=0)
np.save("pca_score_mins.npy", score_mins)
