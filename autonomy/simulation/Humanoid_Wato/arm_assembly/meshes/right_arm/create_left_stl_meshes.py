...
def main() -> int:
    args = parse_args()
    right_dir = Path(__file__).resolve().parent
    left_dir = right_dir.parent / "left_arm"
    left_dir.mkdir(exist_ok=True)
    stl_files = sorted(
        path
        for path in right_dir.glob("*.stl")
        if not path.name.startswith(args.prefix)
    )

    for input_path in stl_files:
        output_path = left_dir / f"{args.prefix}{input_path.name}"
