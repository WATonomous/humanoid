...
from dextrah_rgb.distillation.data_recorder import DataRecorder
...
class PolicyEvaluator:
    ...
    def evaluate(self, num_episodes):
        ...
        for episode in range(num_episodes):
            ...
            if self.is_rnn:
                self.hidden_states = tuple(s.to(self.device) for s in self.model.get_default_rnn_state())
            if hasattr(self.model.a2c_network, "reset_idx"):
                all_env_ids = torch.arange(self.num_envs, device=self.device, dtype=torch.long)
                self.model.a2c_network.reset_idx(all_env_ids)
            ...
            while not dones.all():
                ...
                if len(all_done_indices) > 0:
                    if hasattr(self.model.a2c_network, "reset_idx"):
                        self.model.a2c_network.reset_idx(all_done_indices.squeeze(-1))
