...
This is run_quest_bimanual_teleop.py's ORIGINAL logic, word-for-word, just retargeted at the
wato_arm_v2 / armWithStand URDF instead of the old bimanual URDF -- no smoothing filters, no
adaptive IK damping, no recording, no S/D/T keys, no marker-hiding-for-record logic, none of
run_quest_armv2_teleop.py's later additions. Only things that are facts about the physical
robot/scene (not "logic") were changed: the config module import, the verified-correct gripper
open/closed values (this URDF's prismatic gripper joints run the OPPOSITE direction from the old
one's -- see the import block below), and the already-tuned camera/enclosure/reach constants for
this robot's actual geometry (rest pose, reach envelope). Everything else -- IK approach, gripper
pinch threshold with zero smoothing, R-only recalibration, "snap directly to the DLS solution"
joint application -- is unchanged from the original.
...
armWithStand_v2_cfg.py: midpoint of each gripper's two finger distal tips), not
...
Inside the simulation_isaac container:
    ...
    /workspace/isaaclab/isaaclab.sh -p \\
        /workspace/humanoid/autonomy/simulation/quest_isaac_teleop/run_quest_bimanual_teleop_v2.py \\
        --device cpu
"""
...
# NOTE on the aliasing below: armWithStand_v2_cfg.py's LEFT_*/RIGHT_* constants have the same
# quirk the old bimanual_arm_cfg.py's did (the "L"-suffixed URDF chain is physically the RIGHT
# arm, not left). This file's own left_*/right_* local variables mean something different: "the
# arm driven by the LEFT/RIGHT Quest wrist" (msg.left_wrist / msg.right_wrist), a
# Quest-hand-relative label that is NOT part of that correction. Aliasing here keeps every local
# left_*/right_* variable below pointing at the SAME physical URDF chain it always did -- only
# the upstream symbol names/module changed.
...
# VERIFIED empirically (headless sim, fingertip-gap measurement) -- this URDF's prismatic
# gripper axis runs the OPPOSITE direction from the old bimanual URDF's, so these are flipped
# relative to the original script's _RIGHT_GRIPPER_OPEN/_CLOSED values. Using the old values here
# would open when it should close and vice versa -- this is a physical fact about the new
# hardware, not a logic change.
...
# Reach envelope for THIS arm's geometry (longer chain than the old URDF's) -- already tuned/
# validated live this session, not the old script's original 0.28 value (which was sized for the
# old, shorter-reach arm).
_MAX_REACH_M = 0.5
...
# Enclosure bounds sized for THIS arm's actual reach envelope (already tuned/validated live this
# session for the new, longer-reach geometry) -- not the old script's original bounds, which were
# sized for the old arm's shorter reach.
...
# Real table/box/container assets (not a plain cuboid) -- per explicit live request ("where tf
# is the table and the container and box"), this is the NEW scene (matches ArmV2SceneCfg in
# run_quest_armv2_teleop.py exactly), just paired with the OLD script's teleop control logic.
# Table's local origin sits at its table-TOP surface -- placed so that surface is at world Z=0.
_TABLE_USD_PATH = str(
...
# Box (to grasp) and container (to place it in) -- box placed inside the container.
_BOX_USD_PATH = str(
...
# Home viewpoint already tuned/validated live this session for this arm's actual rest pose and
# enclosure -- not the old script's original pos/quat, which framed the old (different-geometry)
# arm's table setup.
_HEAD_VIEWPOINT_HOME_POS = (0.0905602619765378, 0.04, 0.23809789390566405)  # translate xyz, relative to base_link
_HEAD_VIEWPOINT_HOME_QUAT = (0.9063077870366499, 0.0, 0.42261826174069944, 0.0)  # ~50deg downward pitch, facing +X, no roll
...
# _HEAD_VIEWPOINT_HOME_POS/QUAT above were tuned live assuming this widened FOV (the head is too
# close to the arm to see its full range of motion by moving the camera back -- the stand/mount
# geometry blocks the view first -- so the lens is widened instead). Native RSD455 FOV is much
# narrower (live: focalLength=1.93 at horizontalAperture=3.896) -- without this, the tuned camera
# position/pitch above doesn't see the table/arms (confirmed live: empty ground-grid framing).
...
class BimanualSceneCfg(InteractiveSceneCfg):
    ...
    # ARM_V2_CFG's own default init_state.pos is Z=0 (URDF origin), but this robot physically
    # sits atop a ~1.2m stand -- without this override the whole robot (and the head-mounted
    # camera with it) spawns at ground level instead of on the stand. This is a physical fact
    # about the new URDF, not old-script "logic" -- matches run_quest_armv2_teleop.py's
    # ArmV2SceneCfg, which applies the same override.
