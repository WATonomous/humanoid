...
```bash
mkdir -p src/teleop/quest_teleop/certs
openssl req -x509 -newkey rsa:4096 -keyout src/teleop/quest_teleop/certs/key.pem \
    -out src/teleop/quest_teleop/certs/cert.pem -days 3650 -nodes \
    -subj "/CN=localhost"
```
...
Quest view, both wrist HUDs, and every `--record` dataset camera. It uses sharper 400x300 stereo
eye streams (JPEG quality 70), raises the pose-filter cutoff, and consumes only fresh Quest
samples.
...
Launch recording normally (not headless). `--gain 0.6` makes final grasp alignment less
sensitive; omit it later if you need the original 1:1 near-motion scale:

```bash
./run_quest_bimanual_teleop.sh --record --low-latency --gain 0.6
```
...
| Quest browser shows `localhost terminated connection` | SSL issue or server not running | Confirm Terminal 4 is running; check cert files exist at `src/teleop/quest_teleop/certs/` |
...
precedent, `src/teleop/task_space_controller/task_space_ik.py`
