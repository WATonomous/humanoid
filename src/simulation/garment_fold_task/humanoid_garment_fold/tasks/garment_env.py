...
from humanoid_garment_fold.utils.success_checker_garment import success_checker_garment_fold
from humanoid_garment_fold.assets.scene import MARBLE_BEDROOM_USD_PATH
...
class GarmentEnv(DirectRLEnv):
    ...
    # MODIFIED (WATonomous): upstream's `_get_workspace_pointcloud()` was removed
    # to drop the `lehome.utils.depth_to_pointcloud` dependency (scipy + plotly,
    # unused by the training/eval path). Restore from upstream if you need it.
    ...
    # MODIFIED (WATonomous): upstream's `preprocess_device_action()` was removed
    # to drop the `lehome.devices.action_process` dependency (SO101-leader /
    # keyboard teleop mapping). The pioneer teleop path lives in this repo's
    # src/teleop/ (keyboard, quest, task_space) instead.
