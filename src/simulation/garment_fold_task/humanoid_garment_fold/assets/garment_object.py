...
class GarmentObject(SingleClothPrim):
    ...
    def _get_usd_path(self) -> str:
        ...
        # MODIFIED (WATonomous): honour a real absolute filesystem path first;
        # only fall back to upstream's "/-prefixed == repo-root-relative" rule.
    ...
    def _apply_visual_material(self, material_paths: Sequence[str]):
        ...
        for i, path in enumerate(material_paths):
            # MODIFIED (WATonomous): honour a real absolute path first (see
            # _get_asset_path). Only then apply upstream's repo-root rule.
