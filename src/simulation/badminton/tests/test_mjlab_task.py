...
def env():
    import humanoid_badminton  # noqa: F401
    from humanoid_badminton.env_cfg import make_env_cfg
...
def test_obs_dims(env):
    ...
    # student: proprio 24 + EKF state 6 + prior 3*n_traj + uncertainty 7
    assert obs["student"].shape == (2, 24 + 6 + 3 * n_traj + 7)
...
def test_cork_orientation_tracks_velocity(env):
    ...
    from humanoid_badminton.shuttle_action import quat_z_axis
