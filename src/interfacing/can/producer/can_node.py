...
class CanNode(Node):
    ...
    def __init__(self):
        ...
        # Initialize can core logic for serialization
        self.__can = CanCore(pos_x, pos_y, pos_z, velocity)
    ...
    def __publish_position(self):
        self.__can.update_position()
        msg = Unfiltered()

        msg.data = self.__can.serialize_data()
        msg.valid = True
...
def main(args=None):
    ...
    rclpy.spin(python_can)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    python_can.destroy_node()
    rclpy.shutdown()
