...
void MT6835_Update(void)
{
    ...
    uint32_t raw   = read_raw_21bit();

    angle = raw * (360.0f / 2097152.0f);

    uint8_t burst[6] = { 0xA0, 0x03, 0, 0, 0, 0 };
    cs_low();
    HAL_SPI_TransmitReceive(hspi_, burst, burst, 6, HAL_MAX_DELAY);
    cs_high();
...
float MT6835_GetAngleDeg   (void) { return angle; }
